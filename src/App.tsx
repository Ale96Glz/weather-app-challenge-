    ...
    <>
      <div className="min-h-screen w-full p-4 desktop:px-12 desktop:py-6">
        ...
        <main className="flex flex-col gap-8 mt-12">
                  ...
                  {searchResults.map((result) => (
                    <li key={result.id}>
                      <button
                        type="button"
                        onClick={() => {
                          setQuery(locationLabel(result.name, result.country, result.admin1))
          ...
          {error && !forecast && (
            <div className="flex flex-col items-center gap-4 rounded-2xl bg-neutral-800 px-6 py-10 text-center border border-neutral-600">
              <img src={iconError} alt="" className="h-10 w-10" />
              <p className="text-neutral-200">{error}</p>
              <button
                type="button"
                onClick={retry}
                className="inline-flex items-center gap-2 rounded-xl bg-blue-500 px-4 py-2 text-sm font-medium hover:bg-blue-700"
              >
